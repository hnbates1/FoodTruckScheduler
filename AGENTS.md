...
1. Never delete, truncate, reseed, replace, or reset the production `trucks`, `visits`, `users`, `sessions`, `app_settings`, or traffic snapshot tables as part of startup, deployment, testing, repair, or migration logic.
...
7. Before any intentionally destructive data operation, stop and obtain an explicit instruction from the owner that identifies exactly what should be deleted. Create or verify a backup/export first when supported.
8. Keep `tests/data-preservation.test.mjs` passing. Update its protected storage identifiers only when the owner has explicitly approved a real storage migration.
