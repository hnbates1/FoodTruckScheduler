...
1. Never delete, truncate, reseed, replace, or reset the production `trucks`, `visits`, `truck_comments`, `truck_documents`, `users`, `sessions`, `app_settings`, or traffic snapshot tables as part of startup, deployment, testing, repair, or migration logic.
...
8. Preserve uploaded truck documents and their D1 metadata/R2 objects. A document may be deleted only after an authenticated administrator or manager deliberately uses the document delete control and confirms the action.
9. AI-extracted intake suggestions must never overwrite a live truck record automatically. Apply only fields explicitly selected and confirmed by an authenticated administrator or manager.
10. Before any intentionally destructive data operation, stop and obtain an explicit instruction from the owner that identifies exactly what should be deleted. Create or verify a backup/export first when supported.
11. Keep `tests/data-preservation.test.mjs` passing. Update its protected storage identifiers only when the owner has explicitly approved a real storage migration.
